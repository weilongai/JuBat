...
# Plot
plot(result["time [s]"], result["cell voltage [V]"], label="constant dt", xlabel="time [s]", ylabel="cell voltage [V]",lw=1.5)
plot!(result1["time [s]"], result1["cell voltage [V]"], label="auto dt", linestyle=:dot, lw=2) # shape=:circle
