function  ElectrolytePotential(param::Params, mesh::Mesh, mlen::Int64, variables::Dict{String, Union{Array{Float64},Float64}})
    ...
    coeff_ne = param.EL.kappa(ce_n_gs,T) * param.NE.eps ^ param.NE.brugg
    coeff_sp = param.EL.kappa(ce_sp_gs,T) * param.SP.eps ^ param.SP.brugg
    coeff_pe = param.EL.kappa(ce_p_gs,T) * param.PE.eps ^ param.PE.brugg
