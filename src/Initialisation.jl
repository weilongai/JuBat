function ModelInitialisation(case::Case)
    if isempty(case.opt.y0)
        if case.opt.model == "SPM"
            ...
            csn0 = ones(Float64, Nrn, 1) * case.param.NE.cs_0
            Nrp = case.mesh["positive particle"].nlen
            csp0 = ones(Float64, Nrp, 1) * case.param.PE.cs_0
            y0 = [csn0;  csp0]
        ...
        elseif case.opt.model == "P2D"
            ...
            csp0 = ones(Float64, Nrp, 1) * case.param.PE.cs_0
            ce0 = ones(Float64, Ne, 1) * case.param.EL.ce0
            phie0 = - ones(Float64, Nn, 1) * case.param.NE.U(case.param.NE.cs_0)
            phis_p =  ones(Float64, Nn, 1) * case.param.PE.U(case.param.PE.cs_0) + phie0 # guessed values are not used
